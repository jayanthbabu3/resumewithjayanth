...
export const ModernBusinessTemplate = ({ resumeData, themeColor = "#16a34a", editable = false, onAddBulletPoint, onRemoveBulletPoint }: TemplateProps) => {
  ...
  return (
    <div className="w-full bg-white text-gray-900 p-8 text-[13px] leading-relaxed">
      ...
      <div className="mb-8 relative pl-6" style={{ borderLeft: `6px solid ${themeColor}` }}>
        {editable ? (
          <InlineEditableText
        ...
        {editable ? (
          <InlineEditableText
            ...
            className="text-[15px] text-gray-600 mb-4 block font-semibold"
            as="p"
          />
        ) : (
          <p className="text-[15px] text-gray-600 mb-4 font-semibold">
                  ...
                  {exp.bulletPoints && exp.bulletPoints.length > 0 && (
                    <ul className="mt-3 space-y-2 list-none">
                      {exp.bulletPoints.map((bullet, bulletIndex) => (
                        <li key={bulletIndex} className="text-[13px] text-gray-700 leading-[1.7] flex items-start">
        ...
        <div className="mb-8">
          ...
          {editable ? (
            <InlineEditableSkills
              ...
              renderSkill={(skill) => {
                return skill.name ? (
                  <span className="inline-block px-3 py-1.5 mr-2 mb-2 rounded text-[12px] font-medium text-white" style={{ backgroundColor: themeColor }}>
                    {skill.name}
                  </span>
                ) : null;
              }}
            ...
            <div className="flex flex-wrap">
              {resumeData.skills.map((skill) => (
                skill.name ? (
                  <span key={skill.id} className="px-3 py-1.5 mr-2 mb-2 rounded text-[12px] font-medium text-white" style={{ backgroundColor: themeColor }}>
                    {skill.name}
                  </span>
                ) : null
              ))}
      ...
      {/* Custom Sections */}
      <ModernBusinessCustomSections sections={resumeData.sections} editable={editable} themeColor={themeColor} />
    </div>
  );
};

const ModernBusinessCustomSections = ({ sections, editable, themeColor }: { sections: ResumeData['sections']; editable: boolean; themeColor?: string; }) => {
  ...
  const handleAddSection = (e: React.MouseEvent) => {
    ...
    addArrayItem('sections', { id: Date.now().toString(), title: 'New Section', content: '', items: ['Sample item 1', 'Sample item 2'] });
  };
        ...
        <div key={section.id} className="mb-8 group/section" style={{ pageBreakInside: 'avoid' }}>
          <div className="flex items-center gap-2">
            <h2 className="text-[15px] font-bold mb-4 flex-1" style={{ color: themeColor }}>
              {editable ? <InlineEditableText path={`sections[${index}].title`} value={section.title} className="inline-block" /> : section.title}
            </h2>
            {editable && (
              <button onClick={(e) => handleRemoveSection(e, index)} className="opacity-0 group-hover/section:opacity-100 transition-opacity p-1 rounded hover:bg-red-50" style={{ color: '#ef4444' }} title="Remove Section">
                <X className="h-4 w-4" />
              </button>
            )}
          </div>
          <InlineEditableSectionItems sectionIndex={index} items={section.items || []} content={section.content || ""} editable={editable} itemStyle={{ fontSize: '13px', color: '#374151', lineHeight: '1.7' }} addButtonLabel="Add Item" placeholder="Click to add item..." accentColor={accent} showBullets={false} />
        </div>
      ))}
      {editable && (
        <button onClick={handleAddSection} className="mt-4 flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-md border-2 border-dashed hover:bg-gray-50 transition-colors" style={{ color: accent, borderColor: accent }}>
