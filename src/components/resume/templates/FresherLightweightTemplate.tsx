...
interface FresherLightweightTemplateProps {
...
export const FresherLightweightTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherLightweightTemplateProps) => {
    ...
    <div className="w-full h-full bg-white text-gray-900 p-12">
      ...
      <div className="mb-8">
        ...
        <InlineEditableText
          ...
          className="text-[16px] text-gray-600 mb-4 font-normal"
          editable={editable}
          field="resumeData.personalInfo.title"
        />
        <div className="flex gap-3 text-[13px] text-gray-500 flex-wrap">
      ...
      {resumeData.personalInfo.summary && (
        <div className="mb-8">
          <h2 className="text-[14px] font-semibold mb-3 uppercase tracking-widest" style={{ color: accent }}>
            Summary
          </h2>
          <InlineEditableText
            text={resumeData.personalInfo.summary}
            className="text-[13px] text-gray-700 leading-relaxed font-light"
            editable={editable}
      ...
      {resumeData.education && resumeData.education.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[14px] font-semibold mb-4 uppercase tracking-widest" style={{ color: accent }}>
            Education
          </h2>
          {resumeData.education.map((edu, index) => (
            <div key={edu.id} className="mb-4 last:mb-0">
              ...
              <InlineEditableText
                text={edu.school}
                className="text-[14px] font-normal text-gray-700 mb-1"
                editable={editable}
                field={`resumeData.education[${index}].school`}
              />
              <div className="text-[12px] text-gray-500 flex items-center gap-2 font-light">
                <InlineEditableDate
            ...
            <div key={exp.id} className="mb-5 last:mb-0">
              <div className="flex justify-between items-baseline mb-1">
                ...
                <InlineEditableDate
                  ...
                  className="text-[12px] text-gray-500 font-light"
                  editable={editable}
              ...
              <InlineEditableText
                text={exp.company}
                className="text-[14px] font-normal text-gray-700 mb-2"
                editable={editable}
                field={`resumeData.experience[${index}].company`}
              />
              <InlineEditableList
                items={exp.description.split("\n").filter((item) => item.trim())}
                className="text-[13px] text-gray-700 space-y-1 font-light"
                editable={editable}
