...
interface CloudSolutionsArchitectTemplateProps {
...
export const CloudSolutionsArchitectTemplate = ({
  resumeData,
  themeColor = "#3b82f6",
  editable = false,
}: CloudSolutionsArchitectTemplateProps) => {
  ...
  return (
    <div className="w-full h-full bg-white text-gray-900 p-12">
      <div className="mb-8 pb-6 border-b-4" style={{ borderColor: accent }}>
        <InlineEditableText text={resumeData.personalInfo.fullName} className="text-[40px] font-bold mb-2" style={{ color: accent }} editable={editable} field="resumeData.personalInfo.fullName" />
      ...
      {resumeData.personalInfo.summary && (
        <div className="mb-8">
          <h2 className="text-[17px] font-bold mb-3 uppercase tracking-wider" style={{ color: accent }}>Technical Profile</h2>
      ...
      {resumeData.skills && resumeData.skills.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[17px] font-bold mb-4 uppercase tracking-wider" style={{ color: accent }}>Technical Skills</h2>
          <InlineEditableSkills skills={resumeData.skills} editable={editable} themeColor={accent} />
        </div>
      )}
      {resumeData.experience && resumeData.experience.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[17px] font-bold mb-4 uppercase tracking-wider" style={{ color: accent }}>Professional Experience</h2>
      ...
      {resumeData.education && resumeData.education.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[17px] font-bold mb-4 uppercase tracking-wider" style={{ color: accent }}>Education</h2>
      ...
      {resumeData.sections && resumeData.sections.map((section, sectionIndex) => (
        <div key={section.id} className="mb-8 last:mb-0">
          <h2 className="text-[17px] font-bold mb-4 uppercase tracking-wider" style={{ color: accent }}>
