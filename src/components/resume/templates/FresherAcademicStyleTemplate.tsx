...
interface FresherAcademicStyleTemplateProps {
...
export const FresherAcademicStyleTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherAcademicStyleTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#10b981";

  return (
    <div className="w-full h-full bg-white text-gray-900 p-12" style={{ fontFamily: "Georgia, serif" }}>
      {/* Header - Academic Style */}
      <div className="mb-8 text-center pb-6 border-b-2 border-gray-800">
        <InlineEditableText
          text={resumeData.personalInfo.fullName}
          className="text-[36px] font-bold mb-2 uppercase tracking-wide"
          style={{ color: accent }}
          editable={editable}
        ...
        <InlineEditableText
          ...
          className="text-[16px] text-gray-700 mb-3 italic"
          editable={editable}
          field="resumeData.personalInfo.title"
        />
        <div className="flex justify-center gap-3 text-[12px] text-gray-600 flex-wrap">
      ...
      {resumeData.personalInfo.summary && (
        <div className="mb-8">
          <h2 className="text-[15px] font-bold mb-3 uppercase tracking-widest border-b border-gray-400 pb-1" style={{ color: accent }}>
            Summary
          </h2>
          <InlineEditableText
            text={resumeData.personalInfo.summary}
            className="text-[13px] text-gray-700 leading-relaxed text-justify"
            editable={editable}
      ...
      {resumeData.education && resumeData.education.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[15px] font-bold mb-4 uppercase tracking-widest border-b border-gray-400 pb-1" style={{ color: accent }}>
            Education
          </h2>
          {resumeData.education.map((edu, index) => (
            <div key={edu.id} className="mb-5 last:mb-0">
              ...
              <InlineEditableText
                text={edu.school}
                className="text-[14px] font-semibold text-gray-700 mb-1 italic"
                editable={editable}
        ...
        <div className="mb-8">
          ...
          {resumeData.experience.map((exp, index) => (
            <div key={exp.id} className="mb-5 last:mb-0">
              ...
              <InlineEditableText
                text={exp.company}
                className="text-[14px] font-semibold text-gray-700 mb-2 italic"
                editable={editable}
