...
interface FresherNeonAccentTemplateProps {
...
export const FresherNeonAccentTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherNeonAccentTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#10b981";

  return (
    <div className="w-full h-full bg-white text-gray-900 p-12 relative">
      {/* Neon accent lines */}
      <div className="absolute top-0 left-0 w-1 h-24 opacity-80" style={{ backgroundColor: accent, boxShadow: `0 0 10px ${accent}` }}></div>
      <div className="absolute top-0 right-0 w-1 h-24 opacity-80" style={{ backgroundColor: accent, boxShadow: `0 0 10px ${accent}` }}></div>

      {/* Header */}
      <div className="mb-8 text-center">
        <InlineEditableText
          text={resumeData.personalInfo.fullName}
          className="text-[42px] font-black mb-2 uppercase tracking-tight"
          style={{ color: accent }}
          editable={editable}
        ...
        <InlineEditableText
          ...
          className="text-[18px] text-gray-700 mb-4 font-medium"
          editable={editable}
          field="resumeData.personalInfo.title"
        />
        <div className="flex justify-center gap-4 text-[13px] text-gray-600 flex-wrap">
      ...
      {/* Neon divider */}
      <div className="h-0.5 mb-8 mx-auto w-32 opacity-60" style={{ backgroundColor: accent, boxShadow: `0 0 8px ${accent}` }}></div>

      {/* Summary */}
      {resumeData.personalInfo.summary && (
        <div className="mb-8">
          <h2 className="text-[18px] font-bold mb-3 uppercase tracking-wider pb-2 border-b-2" style={{ color: accent, borderColor: withOpacity(accent, "40") }}>
            About Me
          </h2>
          <InlineEditableText
      ...
      {resumeData.education && resumeData.education.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[18px] font-bold mb-4 uppercase tracking-wider pb-2 border-b-2" style={{ color: accent, borderColor: withOpacity(accent, "40") }}>
            Education
          </h2>
          {resumeData.education.map((edu, index) => (
            <div key={edu.id} className="mb-5 last:mb-0 pl-4 border-l-2" style={{ borderColor: withOpacity(accent, "60") }}>
              <InlineEditableText
                text={edu.degree}
      ...
      {resumeData.experience && resumeData.experience.length > 0 && (
        <div className="mb-8">
          ...
          {resumeData.experience.map((exp, index) => (
            <div key={exp.id} className="mb-5 last:mb-0 pl-4 border-l-2" style={{ borderColor: withOpacity(accent, "60") }}>
        ...
        resumeData.sections.map((section, sectionIndex) => (
          <div key={section.id} className="mb-8 last:mb-0">
            ...
            {section.items?.map((item, itemIndex) => (
              <div key={item.id} className="mb-5 last:mb-0 pl-4 border-l-2" style={{ borderColor: withOpacity(accent, "60") }}>
                {item.title && (
                  <InlineEditableText
                    text={item.title}
                    className="text-[15px] font-bold mb-1"
