...
interface FresherGlassmorphismTemplateProps {
...
export const FresherGlassmorphismTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherGlassmorphismTemplateProps) => {
  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-gray-50 to-gray-100 text-gray-900 p-12">
      ...
      <div className="mb-8 p-6 rounded-2xl backdrop-blur-sm border border-gray-200" style={{ backgroundColor: withOpacity(accent, "10") }}>
        <InlineEditableText
      ...
      {resumeData.personalInfo.summary && (
        <div className="mb-6 p-5 rounded-xl backdrop-blur-sm border border-gray-200 bg-white/50">
      ...
      {resumeData.sections &&
        resumeData.sections.map((section, sectionIndex) => (
          <div key={section.id} className="mb-6 last:mb-0 p-5 rounded-xl backdrop-blur-sm border border-gray-200 bg-white/50">
            <h2 className="text-[16px] font-bold mb-4 uppercase tracking-wider" style={{ color: accent }}>
              <InlineEditableText
                text={section.title}
                editable={editable}
