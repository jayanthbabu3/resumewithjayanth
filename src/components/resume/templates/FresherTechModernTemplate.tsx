...
export const FresherTechModernTemplate = ({ resumeData, themeColor = "#0369a1", editable = false }: TemplateProps) => {
    ...
    <div className="w-full bg-white text-gray-900 p-8" style={{ fontFamily: 'Inter' }}>
      ...
      <div className="mb-8 p-6 rounded-xl" style={{ backgroundColor: `${themeColor}10` }}>
        <div className="flex items-center gap-6">
          {photo && (
            <div className="w-24 h-24 rounded-lg overflow-hidden border-3" style={{ borderColor: themeColor }}>
          ...
          <div className="flex-1">
            ...
            {editable ? (
              <InlineEditableText
                ...
                value={resumeData.personalInfo.title || "Tech Graduate"}
                className="text-[15px] text-gray-700 font-medium block"
                as="p"
              />
            ) : (
              <p className="text-[15px] text-gray-700 font-medium">
                {resumeData.personalInfo.title || "Tech Graduate"}
              </p>
            )}

            <div className="flex gap-6 mt-3 text-[12.5px] text-gray-600">
        ...
        <div className="mb-8">
          ...
          {editable ? (
            <InlineEditableSkills
              ...
              renderSkill={(skill) => (
                <div className="p-3 border-2 rounded-lg text-center" style={{ borderColor: themeColor, backgroundColor: `${themeColor}08` }}>
            ...
            <div className="grid grid-cols-5 gap-3">
              {resumeData.skills.map((skill) => (
                <div key={skill.id} className="p-3 border-2 rounded-lg text-center" style={{ borderColor: themeColor, backgroundColor: `${themeColor}08` }}>
                  <span className="text-[13px] font-medium" style={{ color: themeColor }}>{skill.name}</span>
                </div>
              ))}
            </div>
      ...
      <div className="mb-8">
        ...
        <div className="space-y-5">
          {resumeData.sections.map((section) => (
            <div key={section.id} className="p-5 rounded-lg border-2" style={{ borderColor: `${themeColor}40` }}>
              <h3 className="text-[14px] font-bold mb-2" style={{ color: themeColor }}>{section.title}</h3>
              <p className="text-[13px] text-gray-700 leading-[1.7] whitespace-pre-line mb-3">
                {section.content}
              </p>
              <div className="flex flex-wrap gap-2">
                <span className="px-3 py-1 rounded-full text-[11.5px] font-medium text-white" style={{ backgroundColor: themeColor }}>
                  React
                </span>
                <span className="px-3 py-1 rounded-full text-[11.5px] font-medium text-white" style={{ backgroundColor: themeColor }}>
                  Node.js
                ...
                renderItem={(exp, index) => (
                  <div className="mb-4 p-4 rounded-lg" style={{ backgroundColor: `${themeColor}08` }}>
                    ...
                    <InlineEditableText
                      ...
                      className="text-[12.5px] font-semibold mb-2 block"
                      style={{ color: themeColor }}
                      as="p"
                    />
                    {exp.description && (
              ...
              <div className="space-y-4">
                {resumeData.experience.map((exp) => (
                  <div key={exp.id} className="p-4 rounded-lg" style={{ backgroundColor: `${themeColor}08` }}>
                    ...
                    <p className="text-[12.5px] font-semibold mb-2" style={{ color: themeColor }}>{exp.company || "Company"}</p>
                    {exp.description && (
