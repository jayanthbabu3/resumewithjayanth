...
interface ModernMinimalistUniversalTemplateProps {
...
export const ModernMinimalistUniversalTemplate = ({
  resumeData,
  themeColor = "#6366f1",
  editable = false,
}: ModernMinimalistUniversalTemplateProps) => {
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {resumeData.skills.map((skill, idx) => (
              <span key={idx} className="px-4 py-1.5 text-[12px] rounded" style={{ backgroundColor: accentLight, color: accent }}>
                {skill.name}
              </span>
            ))}
          </div>
