...
interface DottedGridUniversalTemplateProps {
...
export const DottedGridUniversalTemplate = ({
  resumeData,
  themeColor = "#6366f1",
  editable = false,
}: DottedGridUniversalTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#6366f1";
  const accentLight = withOpacity(accent, "10") ?? "#f5f5f5";
  ...
  return (
    <div className="w-full h-full bg-white text-gray-900 p-12 text-[13px] leading-relaxed">
      ...
      {resumeData.personalInfo.summary && (
        <div className="mb-10 p-5 rounded-lg" style={{ backgroundColor: accentLight }}>
          <h2 className="text-[16px] font-bold mb-3" style={{ color: accent }}>Professional Summary</h2>
      ...
      {resumeData.experience && resumeData.experience.length > 0 && (
        <div className="mb-10">
          ...
          {resumeData.experience.map((exp, index) => (
            <div key={index} className="mb-6 last:mb-0 p-4 border-l-4" style={{ borderColor: accent }}>
