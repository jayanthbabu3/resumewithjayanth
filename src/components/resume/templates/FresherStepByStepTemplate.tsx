...
interface FresherStepByStepTemplateProps {
...
export const FresherStepByStepTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherStepByStepTemplateProps) => {
    ...
    <div className="w-full h-full bg-white text-gray-900 p-12">
      ...
      {resumeData.personalInfo.summary && (
        <div className="mb-8 flex gap-4">
          <div className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-[18px]" style={{ backgroundColor: accent }}>
            {++sectionNumber}
          </div>
      ...
      {/* Custom Sections with step numbers */}
      {resumeData.sections &&
        resumeData.sections.map((section, sectionIndex) => (
          <div key={section.id} className="mb-8 last:mb-0 flex gap-4">
            <div className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-[18px]" style={{ backgroundColor: accent }}>
              {++sectionNumber}
            </div>
            <div className="flex-1">
