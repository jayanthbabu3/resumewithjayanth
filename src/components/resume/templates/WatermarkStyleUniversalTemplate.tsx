...
interface WatermarkStyleUniversalTemplateProps {
...
export const WatermarkStyleUniversalTemplate = ({
  resumeData,
  themeColor = "#6366f1",
  editable = false,
}: WatermarkStyleUniversalTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#6366f1";
  const watermark = withOpacity(accent, "05") ?? "#f0f0f0";

  return (
    <div className="w-full h-full bg-white text-gray-900 p-12 text-[13px] leading-relaxed relative">
      {/* Watermark background */}
      <div className="absolute top-1/4 right-10 text-[120px] font-bold opacity-5 pointer-events-none select-none" style={{ color: accent }}>
        {resumeData.personalInfo.fullName.split(' ')[0] || 'RESUME'}
      </div>
      ...
      <div className="relative z-10">
        <div className="mb-10">
          {editable ? (
            <>
              <InlineEditableText path="personalInfo.fullName" value={resumeData.personalInfo.fullName} className="text-[34px] font-bold mb-2 block" style={{ color: accent }} as="h1" />
              <InlineEditableText path="personalInfo.title" value={resumeData.personalInfo.title || "Professional Title"} className="text-[15px] text-gray-700 mb-5 block" as="p" />
            </>
          ) : (
            <>
              <h1 className="text-[34px] font-bold mb-2" style={{ color: accent }}>{resumeData.personalInfo.fullName}</h1>
              <p className="text-[15px] text-gray-700 mb-5">{resumeData.personalInfo.title || "Professional Title"}</p>
            </>
          )}
        ...
        {resumeData.experience && resumeData.experience.length > 0 && (
          <div className="mb-10">
            <h2 className="text-[16px] font-bold mb-4" style={{ color: accent }}>Professional Experience</h2>
