...
interface VisualStorytellerTemplateTemplateProps {
...
export const VisualStorytellerTemplateTemplate = ({
  resumeData,
  themeColor = "#ec4899",
  editable = false,
}: VisualStorytellerTemplateTemplateProps) => {
  ...
  return (
    <div className="w-full h-full bg-white text-gray-900 p-12">
      <div className="mb-8 pb-6 border-b-4" style={{ borderColor: accent }}>
        <InlineEditableText text={resumeData.personalInfo.fullName} className="text-[42px] font-bold mb-2" style={{ color: accent }} editable={editable} field="resumeData.personalInfo.fullName" />
        <InlineEditableText text={resumeData.personalInfo.title} className="text-[18px] text-gray-700 mb-4 font-semibold tracking-wide" editable={editable} field="resumeData.personalInfo.title" />
      ...
      {resumeData.personalInfo.summary && (
        <div className="mb-8">
          <h2 className="text-[17px] font-bold mb-3 uppercase tracking-widest" style={{ color: accent }}>Creative Vision</h2>
      ...
      {resumeData.experience && resumeData.experience.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[17px] font-bold mb-4 uppercase tracking-widest" style={{ color: accent }}>Experience</h2>
          {resumeData.experience.map((exp, index) => (
            <div key={exp.id} className="mb-6 last:mb-0">
              <div className="flex justify-between items-baseline mb-1">
                ...
                <InlineEditableDate date={`${exp.startDate} - ${exp.current ? "Present" : exp.endDate}`} className="text-[12px] text-gray-600 font-medium" editable={editable} field={`resumeData.experience[${index}].startDate`} />
              </div>
      ...
      {resumeData.education && resumeData.education.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[17px] font-bold mb-4 uppercase tracking-widest" style={{ color: accent }}>Education</h2>
      ...
      {resumeData.skills && resumeData.skills.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[17px] font-bold mb-4 uppercase tracking-widest" style={{ color: accent }}>Creative Skills</h2>
      ...
      {resumeData.sections && resumeData.sections.map((section, sectionIndex) => (
        <div key={section.id} className="mb-8 last:mb-0">
          <h2 className="text-[17px] font-bold mb-4 uppercase tracking-widest" style={{ color: accent }}>
