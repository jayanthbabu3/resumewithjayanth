...
interface RetroProfessionalUniversalTemplateProps {
...
export const RetroProfessionalUniversalTemplate = ({
  resumeData,
  themeColor = "#b91c1c",
  editable = false,
}: RetroProfessionalUniversalTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#b91c1c";
  const accentLight = withOpacity(accent, "15") ?? "#f5f5f5";
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {resumeData.skills.map((skill, idx) => (
              <span key={idx} className="px-4 py-1.5 text-[12px] font-medium border-2 rounded" style={{ borderColor: accent, color: accent }}>
                {skill.name}
              </span>
            ))}
          </div>
