...
export const CodeSnippetTemplate = ({
  resumeData,
  themeColor = "#3b82f6",
  editable = false,
}: CodeSnippetTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#3b82f6";

  return (
    <div className="w-full h-full bg-white text-gray-900 p-12">
      {/* Header with code style */}
      <div className="mb-8 p-6 bg-gray-50 border-l-4 rounded" style={{ borderColor: accent, fontFamily: "'Fira Code', monospace" }}>
        <div className="text-[12px] text-gray-500 mb-2">const developer = {'{'}</div>
        <div className="ml-4">
          <div className="mb-1">
        ...
        <div className="text-[12px] text-gray-500 mt-2">{'};'}</div>
      </div>

      {/* Summary */}
      {resumeData.personalInfo.summary && (
        <div className="mb-8">
          <h2 className="text-[16px] font-bold mb-3 font-mono" style={{ color: accent }}>
            // About
          </h2>
          <InlineEditableText
      ...
      {resumeData.experience && resumeData.experience.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[16px] font-bold mb-4 font-mono" style={{ color: accent }}>
            // Experience
          </h2>
