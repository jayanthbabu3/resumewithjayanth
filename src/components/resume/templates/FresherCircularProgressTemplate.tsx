...
interface FresherCircularProgressTemplateProps {
...
export const FresherCircularProgressTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherCircularProgressTemplateProps) => {
  ...
  return (
    <div className="w-full h-full bg-white text-gray-900 p-12">
      ...
      <div className="mb-8 relative">
        <div className="absolute -left-4 top-0 w-20 h-20 rounded-full opacity-20" style={{ backgroundColor: accent }}></div>
        <div className="absolute -left-2 top-2 w-16 h-16 rounded-full opacity-30" style={{ backgroundColor: accent }}></div>
        <div className="absolute left-0 top-4 w-12 h-12 rounded-full" style={{ backgroundColor: accent }}></div>
      ...
      {resumeData.education && resumeData.education.length > 0 && (
        <div className="mb-8">
          ...
          {resumeData.education.map((edu, index) => (
            <div key={edu.id} className="mb-5 last:mb-0 relative pl-6">
              <div className="absolute left-0 top-2 w-2 h-2 rounded-full" style={{ backgroundColor: withOpacity(accent, "70") }}></div>
              <InlineEditableText
                text={edu.degree}
      ...
      {resumeData.experience && resumeData.experience.length > 0 && (
        <div className="mb-8">
          ...
          {resumeData.experience.map((exp, index) => (
            <div key={exp.id} className="mb-5 last:mb-0 relative pl-6">
              <div className="absolute left-0 top-2 w-2 h-2 rounded-full" style={{ backgroundColor: withOpacity(accent, "70") }}></div>
        ...
        resumeData.sections.map((section, sectionIndex) => (
          <div key={section.id} className="mb-8 last:mb-0">
            ...
            {section.items?.map((item, itemIndex) => (
              <div key={item.id} className="mb-5 last:mb-0 relative pl-6">
                <div className="absolute left-0 top-2 w-2 h-2 rounded-full" style={{ backgroundColor: withOpacity(accent, "70") }}></div>
                {item.title && (
                  <InlineEditableText
                    text={item.title}
