...
interface CosmosProfessionalTemplateProps {
...
export const CosmosProfessionalTemplate = ({
  resumeData,
  themeColor = "#4f46e5",
  editable = false,
}: CosmosProfessionalTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#4f46e5";
  const accentLight = withOpacity(accent, "15") ?? "#4f46e515";
  const accentBorder = withOpacity(accent, "33") ?? "#4f46e533";
