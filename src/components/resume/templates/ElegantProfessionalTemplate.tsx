...
export const ElegantProfessionalTemplate = ({ resumeData, themeColor = "#9333ea", editable = false, onAddBulletPoint, onRemoveBulletPoint }: TemplateProps) => {
  ...
  return (
    <div className="w-full bg-white text-gray-900 p-8 text-[13px] leading-relaxed">
      {/* Header */}
      <div className="mb-8 text-center pb-6 border-b" style={{ borderColor: `${themeColor}40` }}>
        {editable ? (
          <InlineEditableText
            path="personalInfo.fullName"
            value={resumeData.personalInfo.fullName || "Your Name"}
            className="text-[38px] font-bold mb-2 block tracking-tight"
            style={{ color: themeColor }}
            as="h1"
          />
        ) : (
          <h1 className="text-[38px] font-bold mb-2 tracking-tight" style={{ color: themeColor }}>
        ...
        {/* Contact */}
        <div className="flex justify-center gap-x-4 text-[12.5px] text-gray-600 flex-wrap">
        ...
        <div className="mb-8">
          ...
          {editable ? (
            <InlineEditableSkills
              ...
              renderSkill={(skill) => {
                return skill.name ? (
                  <span className="inline-block mx-2 mb-2 text-[12.5px] text-gray-700">
                    • {skill.name}
                  </span>
                ) : null;
              }}
            ...
            <div className="flex flex-wrap justify-center">
              {resumeData.skills.map((skill) => (
                skill.name ? (
                  <span key={skill.id} className="mx-2 mb-2 text-[12.5px] text-gray-700">
                    • {skill.name}
                  </span>
                ) : null
              ))}
      ...
      {/* Custom Sections */}
      <ElegantProfessionalCustomSections 
...
const ElegantProfessionalCustomSections = ({ 
  sections, 
  editable, 
  themeColor 
}: { 
