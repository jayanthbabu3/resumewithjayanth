...
interface CodeforgeDeveloperTemplateProps {
...
export const CodeforgeDeveloperTemplate = ({
  resumeData,
  themeColor = "#0f172a",
  editable = false,
}: CodeforgeDeveloperTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#0f172a";
  const accentLight = withOpacity(accent, "15") ?? "#0f172a15";
  const accentBorder = withOpacity(accent, "33") ?? "#0f172a33";
