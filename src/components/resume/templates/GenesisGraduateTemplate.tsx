...
interface GenesisGraduateTemplateProps {
...
export const GenesisGraduateTemplate = ({
  resumeData,
  themeColor = "#0d9488",
  editable = false,
}: GenesisGraduateTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#0d9488";
  const accentLight = withOpacity(accent, "15") ?? "#0d948815";
  const accentBorder = withOpacity(accent, "33") ?? "#0d948833";
