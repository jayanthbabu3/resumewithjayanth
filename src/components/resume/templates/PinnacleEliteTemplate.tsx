...
interface PinnacleEliteTemplateProps {
...
export const PinnacleEliteTemplate = ({
  resumeData,
  themeColor = "#be123c",
  editable = false,
}: PinnacleEliteTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#be123c";
  const accentLight = withOpacity(accent, "15") ?? "#be123c15";
  const accentBorder = withOpacity(accent, "33") ?? "#be123c33";
