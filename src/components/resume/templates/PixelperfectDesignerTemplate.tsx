...
interface PixelperfectDesignerTemplateProps {
...
export const PixelperfectDesignerTemplate = ({
  resumeData,
  themeColor = "#1e293b",
  editable = false,
}: PixelperfectDesignerTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#1e293b";
  const accentLight = withOpacity(accent, "15") ?? "#1e293b15";
  const accentBorder = withOpacity(accent, "33") ?? "#1e293b33";
