...
interface FresherBoxShadowTemplateProps {
...
export const FresherBoxShadowTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherBoxShadowTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#10b981";

  return (
    <div className="w-full h-full bg-gray-100 text-gray-900 p-12">
      {/* Header with shadow */}
      <div className="mb-8 bg-white p-6 rounded-lg" style={{ boxShadow: `0 4px 12px ${withOpacity(accent, "20")}` }}>
        <InlineEditableText
      ...
      {resumeData.sections &&
        resumeData.sections.map((section, sectionIndex) => (
          <div key={section.id} className="mb-6 last:mb-0 bg-white p-5 rounded-lg shadow-md">
            <h2 className="text-[16px] font-bold mb-4 uppercase tracking-wider" style={{ color: accent }}>
              <InlineEditableText
                text={section.title}
                editable={editable}
