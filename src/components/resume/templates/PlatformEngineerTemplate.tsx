...
export const PlatformEngineerTemplate = ({ resumeData, themeColor = "#0891b2", editable = false }: TemplateProps) => {
  ...
  return (
    <div className="w-full min-h-[297mm] bg-white font-['Inter',sans-serif] text-gray-900">
      <div className="bg-gradient-to-r from-cyan-50 to-white border-b-4" style={{ borderColor: accent }}>
        <div className="max-w-4xl mx-auto px-12 pt-11 pb-9">
          <div className="flex items-start justify-between gap-8">
            <div className="flex-1">
              <div className="inline-block px-4 py-1.5 mb-4 text-[11px] font-bold uppercase tracking-wide text-white rounded" style={{ backgroundColor: accent }}>
              ...
              <p className="text-[16px] font-semibold mb-5" style={{ color: accent }}>
                {editable ? <InlineEditableText path="personalInfo.title" value={resumeData.personalInfo.title} placeholder="Platform Engineer | Developer Experience" as="span" /> : resumeData.personalInfo.title}
              </p>
      ...
      <div className="max-w-4xl mx-auto px-12 py-10">
        {(resumeData.personalInfo.summary || editable) && (
          <div className="mb-10">
            <h2 className="text-[15px] font-bold mb-4 text-gray-900 uppercase tracking-wider pb-2 border-b-2" style={{ borderColor: accent }}>Professional Summary</h2>
            <p className="text-[13.5px] text-gray-700 leading-[1.8]">{editable ? <InlineEditableText path="personalInfo.summary" value={resumeData.personalInfo.summary || ""} placeholder="Platform Engineer with 10+ years building developer tools and infrastructure platforms..." multiline as="span" /> : resumeData.personalInfo.summary}</p>
          </div>
        )}
        ...
        {((resumeData.experience && resumeData.experience.length > 0) || editable) && (
          <div className="mb-10">
            ...
            {editable ? <InlineEditableList path="experience" items={resumeData.experience || []} defaultItem={{ position: "Platform Engineer", company: "Tech Company", startDate: new Date().toISOString().split("T")[0], endDate: new Date().toISOString().split("T")[0], current: false, description: "• Built internal developer platform serving 500+ engineers\n• Reduced deployment time from hours to minutes\n• Improved developer productivity by 40%", id: Date.now().toString() }} renderItem={(exp, index) => <div className="mb-7 last:mb-0 bg-gray-50 rounded-lg p-5"><div className="flex justify-between items-start mb-2.5"><div className="flex-1"><h3 className="text-[15px] font-bold text-gray-900"><InlineEditableText path={`experience[${index}].position`} value={exp.position} placeholder="Position Title" as="span" /></h3><p className="text-[14px] font-semibold mt-1" style={{ color: accent }}><InlineEditableText path={`experience[${index}].company`} value={exp.company} placeholder="Company Name" as="span" /></p></div><div className="text-[11.5px] font-semibold whitespace-nowrap ml-4 px-3 py-1.5 rounded-md" style={{ backgroundColor: accent, color: "white" }}><InlineEditableDate path={`experience[${index}].startDate`} value={exp.startDate} formatDisplay={formatDate} className="inline-block" /><span> - </span>{exp.current ? <span>Present</span> : <InlineEditableDate path={`experience[${index}].endDate`} value={exp.endDate} formatDisplay={formatDate} className="inline-block" />}</div></div><div className="text-[13px] text-gray-700 leading-[1.8] whitespace-pre-wrap"><InlineEditableText path={`experience[${index}].description`} value={exp.description} placeholder="• Achievement\n• Accomplishment\n• Impact" multiline as="div" /></div></div>} addButtonLabel="Add Experience" /> : resumeData.experience.map((exp, index) => <div key={index} className="mb-7 last:mb-0 bg-gray-50 rounded-lg p-5"><div className="flex justify-between items-start mb-2.5"><div className="flex-1"><h3 className="text-[15px] font-bold text-gray-900">{exp.position}</h3><p className="text-[14px] font-semibold mt-1" style={{ color: accent }}>{exp.company}</p></div><div className="text-[11.5px] font-semibold whitespace-nowrap ml-4 px-3 py-1.5 rounded-md" style={{ backgroundColor: accent, color: "white" }}>{formatDate(exp.startDate)} - {exp.current ? "Present" : formatDate(exp.endDate)}</div></div><div className="text-[13px] text-gray-700 leading-[1.8] whitespace-pre-wrap">{exp.description}</div></div>)}
          </div>
        )}
