...
interface FresherLeftStripeTemplateProps {
...
export const FresherLeftStripeTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherLeftStripeTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#10b981";

  return (
    <div className="w-full h-full bg-white text-gray-900 flex">
      {/* Left accent stripe */}
      <div className="w-2 flex-shrink-0" style={{ backgroundColor: accent }}></div>
        ...
        {resumeData.education && resumeData.education.length > 0 && (
          <div className="mb-8">
            ...
            {resumeData.education.map((edu, index) => (
              <div key={edu.id} className="mb-5 last:mb-0 pl-4 border-l-4" style={{ borderColor: withOpacity(accent, "40") }}>
                <InlineEditableText
                  text={edu.degree}
        ...
        {resumeData.experience && resumeData.experience.length > 0 && (
          <div className="mb-8">
            ...
            {resumeData.experience.map((exp, index) => (
              <div key={exp.id} className="mb-5 last:mb-0 pl-4 border-l-4" style={{ borderColor: withOpacity(accent, "40") }}>
          ...
          resumeData.sections.map((section, sectionIndex) => (
            <div key={section.id} className="mb-8 last:mb-0">
              ...
              {section.items?.map((item, itemIndex) => (
                <div key={item.id} className="mb-5 last:mb-0 pl-4 border-l-4" style={{ borderColor: withOpacity(accent, "40") }}>
                  {item.title && (
                    <InlineEditableText
                      text={item.title}
                      className="text-[15px] font-bold mb-1"
