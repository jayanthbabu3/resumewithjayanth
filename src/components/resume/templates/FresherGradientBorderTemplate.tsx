...
interface FresherGradientBorderTemplateProps {
...
export const FresherGradientBorderTemplate = ({
  resumeData,
  themeColor = "#10b981",
  editable = false,
}: FresherGradientBorderTemplateProps) => {
  const accent = normalizeHex(themeColor) ?? "#10b981";

  return (
    <div className="w-full h-full bg-white text-gray-900 p-1" style={{ background: `linear-gradient(135deg, ${accent} 0%, ${withOpacity(accent, "60")} 100%)` }}>
      <div className="w-full h-full bg-white p-11">
        {/* Header */}
        <div className="mb-8 text-center">
          <InlineEditableText
            text={resumeData.personalInfo.fullName}
            className="text-[40px] font-black mb-2 uppercase tracking-tight"
            style={{ color: accent }}
            editable={editable}
          ...
          <InlineEditableText
            ...
            className="text-[17px] text-gray-700 mb-4 font-semibold"
            editable={editable}
        ...
        {/* Gradient divider */}
        <div className="h-1 mb-8 rounded" style={{ background: `linear-gradient(90deg, transparent 0%, ${accent} 50%, transparent 100%)` }}></div>
        ...
        {resumeData.education && resumeData.education.length > 0 && (
          <div className="mb-8">
            ...
            {resumeData.education.map((edu, index) => (
              <div key={edu.id} className="mb-5 last:mb-0 pb-5 last:pb-0 border-b last:border-b-0" style={{ borderColor: withOpacity(accent, "20") }}>
                <InlineEditableText
                  text={edu.degree}
        ...
        {resumeData.experience && resumeData.experience.length > 0 && (
          <div className="mb-8">
            ...
            {resumeData.experience.map((exp, index) => (
              <div key={exp.id} className="mb-5 last:mb-0 pb-5 last:pb-0 border-b last:border-b-0" style={{ borderColor: withOpacity(accent, "20") }}>
          ...
          resumeData.sections.map((section, sectionIndex) => (
            <div key={section.id} className="mb-8 last:mb-0">
              ...
              {section.items?.map((item, itemIndex) => (
                <div key={item.id} className="mb-5 last:mb-0 pb-5 last:pb-0 border-b last:border-b-0" style={{ borderColor: withOpacity(accent, "20") }}>
                  {item.title && (
                    <InlineEditableText
                      text={item.title}
                      className="text-[15px] font-bold mb-1"
