#!/usr/bin/env python3
"""
Generate missing PDF templates for existing UI templates
"""

import os
from pathlib import Path

def generate_pdf_template(template_name):
    """Generate PDF template matching the existing UI template"""
...
def main():
    # Get list of UI templates
    ui_templates_dir = Path("src/components/resume/templates")
    pdf_templates_dir = Path("src/components/resume/pdf")

    # Find all UI templates without PDFs
    missing_pdfs = []

    for ui_file in sorted(ui_templates_dir.glob("*.tsx")):
        template_name = ui_file.stem  # Gets filename without extension
        pdf_file = pdf_templates_dir / f"PDF{template_name}.tsx"

        if not pdf_file.exists():
            missing_pdfs.append(template_name)

    print("=" * 80)
    print(f"GENERATING MISSING PDF TEMPLATES (First 100 of {len(missing_pdfs)} total)")
    print("=" * 80)
    print()

    # Generate first 100 missing PDFs
    templates_to_generate = missing_pdfs[:100]

    for i, template_name in enumerate(templates_to_generate, 1):
        pdf_path = pdf_templates_dir / f"PDF{template_name}.tsx"
        pdf_content = generate_pdf_template(template_name)

        with open(pdf_path, 'w') as f:
            f.write(pdf_content)

        print(f"[{i:3d}/100] {template_name:50s} | ✓ Generated")

    print()
    print("=" * 80)
    print(f"✅ Successfully generated 100 PDF templates!")
    print("=" * 80)
    print()
    print(f"REMAINING: {len(missing_pdfs) - 100} PDF templates still need to be generated")
    print()
